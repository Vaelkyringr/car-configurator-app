...
const ConfiguratorMultipleOption: React.FC<ConfiguratorVehicleOptionsHeaderProps> = ({ CarOption }) => {
    ...
    const onOptionSelected = () => {
      ...
      if (!isToggled) {
        dispatch(addExteriorOptionConfiguration({ id: CarOption.id, name: CarOption.title, cost: CarOption.cost }));

      } else {
        dispatch(removeExteriorOptionConfiguration({ id: CarOption.id, name: CarOption.title, cost: CarOption.cost }));
      }
    ...
    return (
      <div className="configurator-vehicle-option fade-in" onClick={onOptionSelected}>
        ...
        <input className="configurator-vehicle-option-checkbox" type="checkbox" checked={isToggled} />
      </div>
    );
  };
